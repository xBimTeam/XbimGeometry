...
namespace Xbim.Geometry.NetCore.Tests
{
    public static partial class IfcMoq
    {
            ...
            double displacementX = 0, double displacementY = 0, double displacementZ = 0)
        {
            ...
            var firstPosition = IfcIfcAxis2Placement3DMock(loc: IfcCartesianPoint3dMock(x: originX, y: originY, z: originZ));
            var secondPosition = IfcIfcAxis2Placement3DMock(loc: IfcCartesianPoint3dMock(x: displacementX + originX, y: displacementY + originY, z: displacementZ + originZ)); //consecutive to the first block in x direction
            ...
            int depth = 2, double displacement = 10)
        {
            ...
            booleanResultMoq.SetupGet(x => x.ExpressType).Returns(metaData.ExpressType(typeof(IfcBooleanResult)));
            var booleanResult = booleanResultMoq.Object;
            var position = IfcIfcAxis2Placement3DMock(loc: IfcCartesianPoint3dMock(x: 0, y: 0, z: 0));
            ...
            for (int i = 0; i < depth; i++)
            {
                ...
                position = IfcIfcAxis2Placement3DMock(loc: IfcCartesianPoint3dMock(x: offset, y: offset, z: offset));
