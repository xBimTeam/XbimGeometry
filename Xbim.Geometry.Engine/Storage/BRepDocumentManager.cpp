...
//#include "MaterialisedShapeDocument.h"
...
namespace Xbim
{
	namespace Geometry
	{
		namespace Storage
		{
			...
			/*IXMaterialisedShapeDocument^ BRepDocumentManager::NewDocument(int ifcId, short ifcTypeId, int ifcShapeId, IXShape^ shape, IXVisualMaterial^ material, double oneMeter)
			{
				return gcnew MaterialisedShapeDocument(this, ifcId, ifcTypeId, ifcShapeId, shape, material, oneMeter);
			}*/
			...
			IXBRepDocument^ BRepDocumentManager::OpenSTEP(System::String^ path)
			{
				...
				try
				{
					...
					if (!stepReader.Perform(pAnsi, aDoc, mp))
				...
				catch (const Standard_Failure& sf)
				{
					...
					sf.Print(strm);
					System::String^ msg = gcnew System::String(strm.str().c_str());
					throw gcnew FileLoadException("Document Manager Error, " + msg);
				}
				finally
				{
					Marshal::FreeHGlobal(p);
