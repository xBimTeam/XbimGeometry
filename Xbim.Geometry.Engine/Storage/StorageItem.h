...
namespace Xbim
{
	namespace Geometry
	{
		namespace Storage
		{
			public ref class StorageItem : XbimHandle<TDF_Label>, IXStorageItem
			{
			private:
			...
			public:
				...
				StorageItem(TDF_Label label) : XbimHandle(new TDF_Label(label))
				{		
				};

				StorageItem() : XbimHandle(new TDF_Label())
				{
				};
				...
				virtual void AddReference(IXStorageItem^ referred, System::Nullable<XbimMatrix3D> transform);
				virtual IXStorageItem^ AddShape(System::String^ name, IXShape^ shape, bool expand);
				virtual void SetPlacement(IIfcObjectPlacement^ placement);
				virtual IXStorageItem^ AddAssembly(System::String^ subAssemblyName);
				virtual IXStorageItem^ AddAssembly(System::String^ subAssemblyName, IIfcObjectPlacement^ placement);
				virtual IXStorageItem^ AddAssembly(System::String^ subAssemblyName, XbimMatrix3D transform);

				virtual IXStorageItem^ AddComponent(System::String^ name, IXStorageItem^ component, IIfcObjectPlacement^ objPlacement, ILogger^ logger);

				virtual IXStorageItem^ AddComponent(System::String^ name, IXStorageItem^ componentItem, XbimMatrix3D transform);

				virtual IXStorageItem^ AddComponent(System::String^ name, int shapeId, IXShape^ shape, IIfcObjectPlacement^ objPlacement, ILogger^ logger);

				virtual IXStorageItem^ AddComponent(System::String^ name, int shapeId, IXShape^ shape);
				virtual IXStorageItem^ AddSubShape(System::String^ name, IXShape^ shape, bool isExistingPart);
				...
				virtual property IEnumerable<IXStorageItem^>^ Components {IEnumerable<IXStorageItem^>^ get(); }
				virtual property IEnumerable<IXStorageItem^>^ SubShapeStorageItems {IEnumerable<IXStorageItem^>^ get(); }
				virtual property IEnumerable<IXShape^>^ SubShapes {IEnumerable<IXShape^>^ get(); }
				virtual property IXStorageItem^ ReferredShape {IXStorageItem^ get(); }
				...
				virtual property IEnumerable<IXStorageItem^>^ ShapesAssignedToMaterial {IEnumerable<IXStorageItem^>^ get(); }
