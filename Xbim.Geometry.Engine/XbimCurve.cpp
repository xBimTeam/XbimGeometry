...
namespace Xbim
{
	namespace Geometry
	{
		...
		void XbimCurve::Init(IIfcPolyline^ curve, ILogger^ logger)
		{
			...
			if (pts->Count != 2)
			{
				XbimGeometryCreator::LogWarning(logger, curve, "Curves can only be created with polylines that have a single segment, inner points have been ignored");
				lastPt = pts->Count - 1;
			}
