...
namespace carve {
  namespace poly {
    ...
    bool Face<ndim>::containsPoint(const vector_t &p, double EPSILON, double EPSILON2) const {
      if (!carve::math::ZERO(carve::geom::distance(plane_eqn, p),EPSILON)) return false;
      // return pointInPolySimple(vertices, projector(), (this->*project)(p));
      return carve::geom2d::pointInPoly(vertices, projector(), face::project(this, p), EPSILON,  EPSILON2).iclass != POINT_OUT;
    }
    ...
    bool Face<ndim>::simpleLineSegmentIntersection(const carve::geom::linesegment<ndim> &line,
      ...
      if (carve::geom2d::pointInPolySimple(vertices, projector(), proj_p, EPSILON)) {
        intersection = p;
        return true;
      }
      return false;
    ...
    IntersectionClass Face<ndim>::lineSegmentIntersection(const carve::geom::linesegment<ndim> &line,
      ...
      carve::geom2d::PolyInclusionInfo pi = carve::geom2d::pointInPoly(vertices, projector(), proj_p,  EPSILON,  EPSILON2);
      switch (pi.iclass) {
      case POINT_VERTEX:
        intersection = p;
