...
namespace carve {
  namespace csg {
        ...
        for (FaceLoop *f = curr.head; f; f = f->next) {
          for (size_t j = 0; j < f->vertices.size(); ++j) {
            if (!classifier.pointOn(vclass, f, j)) {
              PointClass pc = carve::mesh::classifyPoint(poly_a, poly_a_rtree, f->vertices[j]->v, EPSILON, EPSILON2);
    ...
    void performClassifyFaceGroups(FLGroupList &a_loops_grouped,
      ...
      classifier.faceLoopWork(a_loops_grouped, b_loops_grouped, vclass, poly_a, poly_a_rtree, poly_b, poly_b_rtree, EPSILON, EPSILON2);
