...
namespace {
  ...
  inline double orient3d_exact(const vec3 &a,
                               const vec3 &b,
                               const vec3 &c,
                               const vec3 &d) {
  ...
  inline double orient2d_exact(const vec2 &a,
                               const vec2 &b,
                               const vec2 &c) {
  ...
  // returns true if no intersection, based upon edge^a_i and vertex^b_j separating plane.
  // shared vertex case. only one vertex of b (k) to test - the third
  // (shared) vertex will have orientation equal to 0.
  bool sat_plane(const vec3 tri_a[3], const vec3 tri_b[3], size_t i, size_t j, size_t k) {
    double a;
    double b;
    a = orient3d_exact(tri_a[i], tri_a[(i+1)%3], tri_b[j], tri_a[(i+2)%3]);
  ...
  // returns true if no intersection, based upon edge^a_i and vertex^b_j separating plane.
  bool sat_plane(const vec3 tri_a[3], const vec3 tri_b[3], size_t i, size_t j) {
    double a;
    double b_lo, b_hi;
    a = orient3d_exact(tri_a[i], tri_a[(i+1)%3], tri_b[j], tri_a[(i+2)%3]);
    b_lo = orient3d_exact(tri_a[i], tri_a[(i+1)%3], tri_b[j], tri_b[(j+1)%3]);
    b_hi = orient3d_exact(tri_a[i], tri_a[(i+1)%3], tri_b[j], tri_b[(j+2)%3]);
    if (b_lo > b_hi) std::swap(b_lo, b_hi);
    std::cerr << "b_lo: " << b_lo << " b_hi: " << b_hi << " a: " << a << std::endl;
  ...
  //           0 - touching
  //          +1 - intersection
  int sat_edge(const vec2 tri_a[3], const vec2 tri_b[3], size_t i) {
    return
      max3(dbl_sign(orient2d_exact(tri_a[i], tri_a[(i+1)%3], tri_b[0])),
  ...
  //           0 - touching
  //          +1 - intersection
  int sat_edge(const vec2 tri_a[3], const vec2 tri_b[3], size_t i, size_t j) {
    return
      std::max(dbl_sign(orient2d_exact(tri_a[i], tri_a[(i+1)%3], tri_b[(j+1)%3])),
               dbl_sign(orient2d_exact(tri_a[i], tri_a[(i+1)%3], tri_b[(j+2)%3])));
  }
  ...
  int test_sharing(const carve::geom::vector<ndim> tri_a[3],
                   const carve::geom::vector<ndim> tri_b[3],
                   size_t &ia,
                   size_t &ib) {
    size_t a, b;

    for (b = 0; b < 3; ++b) {
      if (tri_a[0] == tri_b[b] && tri_a[1] == tri_b[(b+1)%3] && tri_a[2] == tri_b[(b+2)%3]) { ia = 0; ib = b; return +3; }
      if (tri_a[0] == tri_b[b] && tri_a[1] == tri_b[(b+2)%3] && tri_a[2] == tri_b[(b+1)%3]) { ia = 0; ib = b; return -3; }
    }

    for (a = 0; a < 3; ++a) {
      for (b = 0; b < 3; ++b) {
        if (tri_a[a] == tri_b[b] && tri_a[(a+1)%3] == tri_b[(b+1)%3]) { ia = a; ib = b; return +2; }
        if (tri_a[a] == tri_b[b] && tri_a[(a+1)%3] == tri_b[(b+2)%3]) { ia = a; ib = b; return -2; }
      }
    }

    for (a = 0; a < 3; ++a) {
      for (b = 0; b < 3; ++b) {
        if (tri_a[a] == tri_b[b]) { ia = a; ib = b; return +1; }
      }
    }
  ...
  void project_triangles(const vec3 tri_a[3], const vec3 tri_b[3],
                        vec2 ptri_a[3], vec2 ptri_b[3]) {
    unsigned a1, a2;
  ...
  carve::geom::TriangleInt triangle_intersection_coplanar(const vec3 tri_a[3], const vec3 tri_b[3]) {
  ...
  bool triangle_intersection_coplanar_simple(const vec3 tri_a[3], const vec3 tri_b[3]) {
    vec2 ptri_a[3], ptri_b[3];
    project_triangles(tri_a, tri_b, ptri_a, ptri_b);
  ...
  carve::geom::TriangleIntType triangle_intersection_coplanar_exact(const vec3 tri_a[3], const vec3 tri_b[3]) {
    vec2 ptri_a[3], ptri_b[3];
    project_triangles(tri_a, tri_b, ptri_a, ptri_b);
  ...
  bool touches_edge(const vec3 tri_a[3], const int nb[3], const vec3 tri_b[3]) {
    ...
    return triangle_linesegment_intersection_exact(ptri_a, pline_b) != carve::geom::TR_TYPE_NONE;
  }
  ...
  bool touches_vert(const vec3 tri_a[3], const int nb[3], const vec3 tri_b[3]) {
    ...
    return triangle_point_intersection_exact(ptri_a, ppt_b) != carve::geom::TR_TYPE_NONE;
  }
...
namespace carve {
  namespace geom {
    ...
    TriangleIntType triangle_intersection_exact(const vec3 tri_a[3], const vec3 tri_b[3]) {
