...
namespace {
  ...
  inline void facesForVertex(carve::mesh::MeshSet<3>::vertex_t *v,
    ...
    if (vi != ve.end()) {
      for (carve::csg::detail::VEVecMap::value_type::second_type::const_iterator i = (*vi).second.begin(); i != (*vi).second.end(); ++i) {
        faces.insert((*i)->face);
      }
    }
  }
...
void carve::csg::CSG::groupIntersections() {
  ...
  for (size_t i = 0, l = vertices.size(); i != l; ++i) {
    ...
    for (size_t j = 0; j < out.size(); ++j) {
      if (vertices[i] != out[j] && carve::geom::equal(vertices[i]->v, out[j]->v,EPSILON)) {
#if defined(CARVE_DEBUG)
...
void carve::csg::CSG::intersectingFacePairs(detail::Data &data) {
  ...
  for (VertexIntersections::const_iterator i = vertex_intersections.begin(), ie = vertex_intersections.end(); i != ie; ++i) {
    ...
    for (VertexIntersections::value_type::second_type::const_iterator j = (*i).second.begin(), je = (*i).second.end(); j != je; ++j) {
...
void carve::csg::CSG::_generateVertexVertexIntersections(meshset_t::vertex_t *va,
                                                         meshset_t::edge_t *eb, double EPSILON2) {
...
void carve::csg::CSG::generateVertexVertexIntersections(meshset_t::face_t *a,
                                                        const std::vector<meshset_t::face_t *> &b, double EPSILON2) {
  ...
  do {
    for (size_t i = 0; i < b.size(); ++i) {
      ...
      eb = t->edge;
      do {
        _generateVertexVertexIntersections(ea->v1(), eb, EPSILON2);
        eb = eb->next;
      } while (eb != t->edge);
    }
    ea = ea->next;
...
void carve::csg::CSG::_generateVertexEdgeIntersections(meshset_t::vertex_t *va,
                                                       meshset_t::edge_t *eb, double EPSILON, double EPSILON2) {
  ...
  if (eb_aabb.maxAxisSeparation(va->v) > EPSILON) {
    return;
  }
...
void carve::csg::CSG::generateVertexEdgeIntersections(meshset_t::face_t *a,
                                                      const std::vector<meshset_t::face_t *> &b, double EPSILON, double EPSILON2) {
  meshset_t::edge_t *ea, *eb;
  ...
  do {
    for (size_t i = 0; i < b.size(); ++i) {
      ...
      eb = t->edge;
      do {
        _generateVertexEdgeIntersections(ea->v1(), eb,  EPSILON,  EPSILON2);
        eb = eb->next;
      } while (eb != t->edge);
    }
    ea = ea->next;
...
void carve::csg::CSG::_generateEdgeEdgeIntersections(meshset_t::edge_t *ea,
                                                     meshset_t::edge_t *eb, double EPSILON) {
...
void carve::csg::CSG::generateEdgeEdgeIntersections(meshset_t::face_t *a,
                                                    const std::vector<meshset_t::face_t *> &b, double EPSILON) {
  meshset_t::edge_t *ea, *eb;
  ...
  do {
    for (size_t i = 0; i < b.size(); ++i) {
      ...
      eb = t->edge;
      do {
        _generateEdgeEdgeIntersections(ea, eb, EPSILON);
        eb = eb->next;
      } while (eb != t->edge);
    }
    ea = ea->next;
...
void carve::csg::CSG::_generateVertexFaceIntersections(meshset_t::face_t *fa,
  ...
  if (fabs(d1) < EPSILON &&
      fa->containsPoint(eb->v1()->v, EPSILON, EPSILON2)) {
...
void carve::csg::CSG::generateVertexFaceIntersections(meshset_t::face_t *a,
  ...
  for (size_t i = 0; i < b.size(); ++i) {
    ...
    eb = t->edge;
    do {
      _generateVertexFaceIntersections(a, eb, EPSILON, EPSILON2);
      eb = eb->next;
    } while (eb != t->edge);
  }
}
...
void carve::csg::CSG::_generateEdgeFaceIntersections(meshset_t::face_t *fa,
  ...
  if (fa->simpleLineSegmentIntersection(carve::geom3d::LineSegment(eb->v1()->v, eb->v2()->v), _p, EPSILON)) {
...
void carve::csg::CSG::generateEdgeFaceIntersections(meshset_t::face_t *a,
  ...
  for (size_t i = 0; i < b.size(); ++i) {
    ...
    eb = t->edge;
    do {
      _generateEdgeFaceIntersections(a, eb, EPSILON);
      eb = eb->next;
    } while (eb != t->edge);
  }
}
...
void carve::csg::CSG::generateIntersections(meshset_t *a,
                                            ...
                                            detail::Data &data, double EPSILON, double EPSILON2) {
  face_pairs_t face_pairs;
  generateIntersectionCandidates(a, a_rtree, b, b_rtree, face_pairs, EPSILON);
...
double carve::csg::CSG::getPrecision() {return precision;};
double carve::csg::CSG::getPrecision2(){return precision2;};
...
void carve::csg::CSG::makeFaceEdges(carve::csg::EdgeClassification &eclass,
  ...
  for (detail::FVSMap::const_iterator
    ...
    for (detail::FSet::const_iterator
      ...
      if (!facesAreCoplanar(face_a, face_b, EPSILON)) {
...
static void checkFaceLoopIntegrity(carve::csg::FaceLoopList &fll) {
  ...
  std::unordered_map<carve::csg::V2, int, carve::csg::VertexHasher> counts;
  ...
  for (std::unordered_map<carve::csg::V2, int, carve::csg::VertexHasher>::const_iterator
...
void carve::csg::CSG::calc(meshset_t *a,
  ...
  generateIntersections(a, a_rtree, b, b_rtree, data,  EPSILON,  EPSILON2);
  ...
  makeFaceEdges(eclass, data, EPSILON, EPSILON2);

#if defined(CARVE_DEBUG)
  std::cerr << "generateFaceLoops" << std::endl;
#endif
  a_edge_count = generateFaceLoops(a, data, a_face_loops, EPSILON, EPSILON2);
  b_edge_count = generateFaceLoops(b, data, b_face_loops, EPSILON, EPSILON2);
