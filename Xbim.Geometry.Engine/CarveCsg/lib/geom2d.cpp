...
namespace carve {
  namespace geom2d {
    ...
    LineIntersectionInfo lineSegmentIntersection(const P2 &l1v1, const P2 &l1v2,
                                                 const P2 &l2v1, const P2 &l2v2, double EPSILON, double EPSILON2) {
      geom::aabb<2> l1_aabb, l2_aabb;
      l1_aabb.fit(l1v1, l1v2);
      l2_aabb.fit(l2v1, l2v2);
      ...
      if (carve::geom::equal(l1v1, l1v2,EPSILON) || carve::geom::equal(l2v1, l2v2,EPSILON)) {
      ...
      if (carve::math::ZERO(u_d,EPSILON)) {
        if (carve::math::ZERO(ua_n,EPSILON)) {
          if (carve::geom::equal(l1v2, l2v1,EPSILON)) {
            return LineIntersectionInfo(INTERSECTION_PP, l1v2, 1, 2);
          }
          if (carve::geom::equal(l1v1, l2v2,EPSILON)) {
    ...
    LineIntersectionInfo lineSegmentIntersection(const LineSegment2 &l1,
                                                 const LineSegment2 &l2, double EPSILON, double EPSILON2) {
      return lineSegmentIntersection(l1.v1, l1.v2, l2.v1, l2.v2,  EPSILON,  EPSILON2);
    }
    ...
    bool pointInPolySimple(const P2Vector &points, const P2 &p, double EPSILON) {
      return pointInPolySimple(points, p2_adapt_ident(), p, EPSILON);
    }

    PolyInclusionInfo pointInPoly(const P2Vector &points, const P2 &p, double EPSILON, double EPSILON2) {
      return pointInPoly(points, p2_adapt_ident(), p, EPSILON, EPSILON2);
    }
    ...
    int lineSegmentPolyIntersections(const P2Vector &points,
      ...
      for (P2Vector::size_type i = 0, l = points.size(); i < l; i++) {
        ...
        LineIntersectionInfo e =
          lineSegmentIntersection(LineSegment2(points[i], points[j]), line, EPSILON,  EPSILON2);
    ...
    int sortedLineSegmentPolyIntersections(const P2Vector &points,
      ...
      int count = lineSegmentPolyIntersections(points, line, EPSILON,  EPSILON2, out);
      if (swapped) {
    ...
    bool pickContainedPoint(const std::vector<P2> &poly, P2 &result, double EPSILON) {
      return pickContainedPoint(poly, p2_adapt_ident(), result, EPSILON);
    }
