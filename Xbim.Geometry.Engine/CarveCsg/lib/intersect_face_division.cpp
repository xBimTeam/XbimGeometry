...
//#endif
  ...
  static void computeContainment(carve::mesh::MeshSet<3>::face_t *face,
    ...
    for (unsigned i = 0; i < hole_loops.size(); ++i) {
      ...
      for (unsigned j = 0; j < face_loops.size(); ++j) {
        ...
        if (carve::geom2d::pointInPolySimple(face_loops_projected[j], test_p, EPSILON)) {
#if defined(CARVE_DEBUG)
  ...
  static void mergeFacesAndHoles(carve::mesh::MeshSet<3>::face_t *face,
    ...
    computeContainment(face, face_loops, hole_loops, containing_faces, hole_shared_vertices, EPSILON);
...
#endif
#if defined(CARVE_DEBUG_WRITE_PLY_DATA)
    dumpFacesAndHoles(f_loops.begin(), f_loops.end(), h_loops.begin(), h_loops.end(), "post_merge.ply");
#endif
  ...
  bool processCrossingEdges(carve::mesh::MeshSet<3>::face_t *face,
                            ...
                            std::list<std::vector<carve::mesh::MeshSet<3>::vertex_t *> > &face_loops_out, double EPSILON, double EPSILON2) {
    ...
    for (size_t i = 0; i < cross.size(); ++i) {
      ...
      if (i < cross.size() - 1 &&
		...
		if(pos <= e1_1)
		{
			...
			//CARVE_FAIL("Error processing edges");
		}
    ...
    for (size_t i = 0; i < divided_base_loop.size(); ++i) {
      ...
      if (inc.size()) {
        ...
        if (hole_loops.size()) {
          mergeFacesAndHoles(face, face_loops, hole_loops, hooks, EPSILON);
        }
  ...
  void generateOneFaceLoop(carve::mesh::MeshSet<3>::face_t *face,
                           ...
                           const carve::csg::VertexIntersections &vertex_intersections,
                           carve::csg::CSG::Hooks &hooks,
                           std::list<std::vector<carve::mesh::MeshSet<3>::vertex_t *> > &face_loops, double EPSILON, double EPSILON2) {
    using namespace carve::csg;
    ...
    if (!paths.size()) {
      ...
      if (processCrossingEdges(face, vertex_intersections, hooks, base_loop, paths, face_loops, EPSILON, EPSILON2)) {
        // Worked.
      } else {
...
size_t carve::csg::CSG::generateFaceLoops(carve::mesh::MeshSet<3> *poly,
                                          ...
                                          FaceLoopList &face_loops_out, double EPSILON, double EPSILON2) {
  ...
  for (carve::mesh::MeshSet<3>::face_iter i = poly->faceBegin(); i != poly->faceEnd(); ++i) {
    ...
    generateOneFaceLoop(face, data, vertex_intersections, hooks, face_loops,  EPSILON,  EPSILON2);
