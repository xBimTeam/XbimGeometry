...
namespace carve {
  namespace mesh {
    ...
    template<unsigned ndim>
    bool Face<ndim>::containsPoint(const vector_t &p, double EPSILON, double EPSILON2) const {
      if (!carve::math::ZERO(carve::geom::distance(plane, p),EPSILON)) return false;
      // return pointInPolySimple(vertices, projector(), (this->*project)(p));
      std::vector<carve::geom::vector<2> > verts;
      getProjectedVertices(verts);
      return carve::geom2d::pointInPoly(verts, project(p),  EPSILON,  EPSILON2).iclass != carve::POINT_OUT;
    }



    template<unsigned ndim>
    bool Face<ndim>::containsPointInProjection(const vector_t &p, double EPSILON, double EPSILON2) const {
      std::vector<carve::geom::vector<2> > verts;
      getProjectedVertices(verts);
      return carve::geom2d::pointInPoly(verts, project(p),  EPSILON,  EPSILON2).iclass != carve::POINT_OUT;
    }
    ...
    bool Face<ndim>::simpleLineSegmentIntersection(
      ...
      if (!line.OK(EPSILON)) return false;

      carve::mesh::MeshSet<3>::vertex_t::vector_t p;
      carve::IntersectionClass intersects =
        carve::geom3d::lineSegmentPlaneIntersection(plane, line, p,EPSILON);
      ...
      std::vector<carve::geom::vector<2> > verts;
      getProjectedVertices(verts);
      if (carve::geom2d::pointInPolySimple(verts, project(p), EPSILON)) {
        intersection = p;
        return true;
      }
      return false;
    ...
    IntersectionClass Face<ndim>::lineSegmentIntersection(const carve::geom::linesegment<ndim> &line,
      ...
      std::vector<carve::geom::vector<2> > verts;
      getProjectedVertices(verts);
      carve::geom2d::PolyInclusionInfo pi = carve::geom2d::pointInPoly(verts, project(p),  EPSILON,  EPSILON2);
      switch (pi.iclass) {
      case POINT_VERTEX:
        intersection = p;
...
nextEdge:
      ...
      void FaceStitcher::matchOrderedEdges(const std::vector<std::vector<EdgeOrderData> >::iterator begin,
                                           ...
                                           std::vector<std::vector<Edge<3> *> > &erev) {
		  typedef std::unordered_map<std::pair<size_t, size_t>, size_t, carve::csg::SizeTHasher> pair_counts_t;
        for (;;) {
  ...
  mesh::MeshSet<3> *meshFromPolyhedron(const poly::Polyhedron *poly, int manifold_id) {
    ...
    std::unordered_map<std::pair<size_t, size_t>, std::list<edge_t *>, carve::csg::SizeTHasher > vertex_to_edge;
  ...
  static void copyMeshFaces(const mesh::Mesh<3> *mesh,
	  ...
	  std::unordered_map<std::pair<size_t, size_t>, std::list<mesh::Edge<3> *>, carve::csg::SizeTHasher > &edges,
 ...
 ////   size_t n_edges = 0;
	////for (std::unordered_map<std::pair<size_t, size_t>, std::list<mesh::Edge<3> *>, carve::csg::SizeTHasher >::iterator i = edges.begin(); i != edges.end(); ++i) {
...
carve::PointClass carve::mesh::classifyPoint(
  ...
  for (size_t i = 0; i < near_faces.size(); i++) {
    ...
    if (near_faces[i]->containsPoint(v,  EPSILON,  EPSILON2)) {
#if defined(DEBUG_CONTAINS_VERTEX)
