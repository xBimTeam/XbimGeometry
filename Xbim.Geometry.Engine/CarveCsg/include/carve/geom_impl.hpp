...
namespace carve {
  namespace geom {
    ...
    template<unsigned ndim>
    bool equal(const vector<ndim> &a, const vector<ndim> &b, double EPSILON) {
      return (b - a).isZero(EPSILON);
    }
    ...
    template<unsigned ndim>
    bool ray<ndim>::OK(double EPSILON) const {
      return !D.isZero(EPSILON);
    }
    ...
    template<unsigned ndim>
    bool linesegment<ndim>::OK(double EPSILON) const {
      return !half_length.isZero(EPSILON);
    }
