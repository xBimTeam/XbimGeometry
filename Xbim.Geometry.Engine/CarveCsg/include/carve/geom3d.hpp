...
namespace carve {
  namespace geom3d {
    ...
    static inline double antiClockwiseAngle(const Vector &from, const Vector &to, const Vector &orient,double EPSILON) {
      double dp = dot(from, to);
      Vector cp = cross(from, to);
      if (cp.isZero(EPSILON)) {
        if (dp < 0) {
          return M_PI;
        } else {
          return 0.0;
    ...
    static inline double antiClockwiseOrdering(const Vector &from, const Vector &to, const Vector &orient,double EPSILON) {
      double dp = dot(from, to);
      Vector cp = cross(from, to);
      if (cp.isZero(EPSILON)) {
        if (dp < 0) {
