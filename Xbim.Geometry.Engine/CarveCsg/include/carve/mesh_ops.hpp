  ...
  namespace mesh {
    namespace detail {
      ...
      struct TriangulationData {
        ...
        template<typename out_iter_t>
        bool splitAndResume(VertexInfo *begin, out_iter_t out, double EPSILON) {
          diag_t diag;
        ...
        template<typename out_iter_t>
        bool doTriangulate(VertexInfo *begin, out_iter_t out, double EPSILON);
...
<svg\n\
  ...
  xml:space=\"preserve\">\n";
      ...
      template<unsigned ndim, typename proj_t>
      template<typename out_iter_t>
      bool TriangulationData<ndim, proj_t>::doTriangulate(VertexInfo *begin, out_iter_t out, double EPSILON) {
        EarQueue vq(*this);
        ...
        if (remain > 3) {
          return splitAndResume(begin, out, EPSILON);
        }
