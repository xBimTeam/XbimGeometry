...
namespace carve {
  namespace triangulate {
    namespace detail {
      ...
      bool splitAndResume(vertex_info *begin, std::vector<carve::triangulate::tri_idx> &result, double EPSILON);

      bool doTriangulate(vertex_info *begin, std::vector<carve::triangulate::tri_idx> &result, double EPSILON);
      ...
      struct tri_pair_t {
        ...
        double calc(const project_t &project,
                    ...
                    distance_calc_t dist, double EPSILON, double EPSILON2) {
          unsigned ai, bi;
      ...
      struct tri_pairs_t {
        ...
        void updateEdge(tri_pair_t *tp,
                        ...
                        size_t &n, double EPSILON, double EPSILON2) {
          double old_score = tp->score;
          double new_score = tp->calc(project, poly, dist,  EPSILON,  EPSILON2);
#if defined(CARVE_DEBUG)
        ...
        void flip(const project_t &project,
          ...
          tp2 = get(perim[0]);
          if (tp2 != NULL) {
            updateEdge(tp2, project, poly, dist, edges, n,  EPSILON,  EPSILON2);
          }
    ...
    void improve(const project_t &project,
      ...
      while (n) {
        tri_pairs.flip(project, poly, dist, edges, n,  EPSILON,  EPSILON2);
      }
    ...
    void improve(const project_t &project,
      ...
      improve(project, poly, carve::geom::distance_functor(), result, double EPSILON, double EPSILON2);
    }
