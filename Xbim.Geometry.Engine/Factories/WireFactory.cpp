...
int WireFactory::Make(
	int contextId, const char * contextLabel,
	...
	TopoDS_Wire& resultWire)
{
	try
	{
		LoggingService::LogInformation(contextId, contextLabel, "Processing Commenced");
	...
	nextEdgeVertex, const TopoDS_Vertex& wireVertexToJoin, gp_Pnt wirePointToJoin, double distance)
{
	...
	if (adjust)
	{
		...
		builder.UpdateVertex(wireVertexToJoin, PC, maxtol);
	}
	TopoDS_Vertex firstEdgeVertex = wireVertexToJoin;
	firstEdgeVertex.Orientation(TopAbs_FORWARD);
	builder.Add(myEdge, firstEdgeVertex);
	TopoDS_Vertex nextEdgeVertexCopy = nextEdgeVertex;
	nextEdgeVertexCopy.Orientation(TopAbs_REVERSED);
	builder.Add(myEdge, nextEdgeVertexCopy);
	builder.Transfert(edgeToAdd, myEdge, edgeVertexToJoin, firstEdgeVertex);
	builder.Add(resultWire, myEdge);
}
