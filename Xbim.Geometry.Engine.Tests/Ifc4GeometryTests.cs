...
namespace Xbim.Geometry.Engine.Tests
{
    public class Ifc4GeometryTests
    {
        ...
        public void IfcTriangulatedFaceSetTest()
        {
            ...
            basinTess.Should().NotBeNull();
            tessellator.CanMesh(basinTess).Should().BeTrue();
            var geom = tessellator.Mesh(basinTess);
            ((int)(geom.BoundingBox.Volume)).Should().Be(23913891);
        }
        ...
        public void IfcTriangulatedFaceSetWithNormalsTest()
        {
            ...
            var tessellator = new XbimTessellator(store, XbimGeometryType.PolyhedronBinary);
            columnTess.Should().NotBeNull();

            tessellator.CanMesh(columnTess).Should().BeTrue();
            var geom = tessellator.Mesh(columnTess);
            ((int)(geom.BoundingBox.Volume)).Should().Be(7680);
        }
        ...
        public void IfcTriangulatedFaceSetWithColoursTest()
        {
            ...
            var tessellator = new XbimTessellator(store, XbimGeometryType.PolyhedronBinary);
            tessellator.CanMesh(triangulatedFaceSet).Should().BeTrue();
            var geom = tessellator.Mesh(triangulatedFaceSet);
            ((int)(geom.BoundingBox.Volume)).Should().Be(2000000000);
        }
        ...
        public void IfcTriangulatedFaceSetSimpleBinaryTest()
        {
            ...
            var sum = myShapeTriangulation.Faces.Sum(t => t.TriangleCount);
            var expteced = Convert.ToInt32(triangulatedFaceSet.NumberOfTriangles.Value);
            sum.Should().Be(expteced);
        }
