    ...
    public class WexBIMTests
    {
        ...
        public void ReadAndWriteWexBimFile()
        {
            using (var m = IfcStore.Open("TestFiles\\SampleHouse4.ifc"))
            {
                ...
                wall.Should().NotBeNull("No IfcWall found in file");
                
                var brep = m.Instances[1213] as IIfcFacetedBrep;
                brep.Should().NotBeNull("No IfcFacetedBrep found in file");
                
                var styledItem = brep.StyledByItem.FirstOrDefault(); //this is not fast do not do on large models
                styledItem.Should().NotBeNull("No StyledItem found in file");

                var repContext = m.Instances[100] as IIfcGeometricRepresentationContext;
                repContext.Should().NotBeNull("No GeometricRepresentationContext found in file");
            ...
            using (var br = new BinaryReader(fs))
            {
                ...
                magicNumber.Should().Be(IfcStoreGeometryExtensions.WexBimId);
                ...
                for (int i = 0; i < shapeCount; i++)
                {
                    ...
                    shapeRepetition.Should().BeGreaterThan(0);
                    if (shapeRepetition > 1)
                    {
                        ...
                        triangulation.Vertices.Count.Should().BeGreaterThan(0, "Number of vertices should be greater than zero");
