...
namespace GeometryTests
{
    ...
    public class IfcSweptSolidTests
    {
        ...
        [TestMethod]
        public void IfcSweptDisk_With_IfcCompositeCurveNegativeDirection()
        {
            using (var eventTrace = LoggerFactory.CreateEventTrace())
            {
                using (var m = new XbimModel())
                {
                    m.CreateFrom("SolidTestFiles\\SweptExtrusion.ifc", null, null, true, true);
                    ...
                    IfcCsgTests.GeneralTest(solid);
                    Assert.IsTrue(solid.Faces.Count() == 9, "Swept disk solids along this composite curve should have 9 faces");
                }
            }
        }
